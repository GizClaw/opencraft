...
make build-macos                            # package the macOS app for this machine
...
`make build-macos`, `make build-linux`, `make build-windows`, and
`make build-windows-installer` wrap the corresponding Wails builds. The macOS
target pins `GOTOOLCHAIN` to the Go version in `go.mod`; newer local Go
toolchains (e.g. Homebrew Go 1.27) otherwise link with a newer macOS minimum
than Wails expects. The Windows binary cross-compiles from any host, and the
installer needs `makensis`, e.g. `brew install nsis`.
