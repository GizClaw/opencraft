...
func main() {
	...
	if err != nil {
		fatal(1, "opencraft: user config dir: %v", err)
	}
	// Explicit reconfiguration: `opencraft setup` rewrites the user
	// configuration layer (inference wiring) and exits.
	if flag.Arg(0) == "setup" {
		if err := setup.Run(userDir); err != nil {
			fatal(1, "opencraft: setup: %v", err)
		}
		return
	}
	// First run: inference wiring lives in the user layer
	// (~/.opencraft/config/opencraft.yaml), generated by the wizard.
	// Block before loading so the runtime never starts half-configured;
	// a non-interactive `run` reports the missing config instead.
	if *configPath == "" {
		needed, err := setup.Needed(userDir)
		if err != nil {
			fatal(1, "opencraft: check config: %v", err)
		}
		if needed {
			if flag.Arg(0) == "run" {
				// Usage error: must be visible on stderr, not just the
				// telemetry log the TUI path relies on.
				fmt.Fprintln(os.Stderr, "opencraft: 未配置推理：请先运行 `opencraft setup` 或交互式 `opencraft` 完成首次配置")
				os.Exit(2)
			}
			if err := setup.Run(userDir); err != nil {
				fatal(1, "opencraft: setup: %v", err)
