...
func TestImportAbortRemovesMemoryRows(t *testing.T) {
	store, err := New(filepath.Join(t.TempDir(), "sessions"), 40)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Insert a memory item exactly like the SQLite adapter does.
	st := store.State()
	if err := st.AppendItems(ctx, []state.Item{{
		ID:        id + ":source:0",
		ThreadID:  id,
		TurnID:    "source",
		Seq:       0,
	...
	if err := store.AbortImport(ctx, id); err != nil {
		t.Fatalf("AbortImport: %v", err)
	}
	if store.Exists(id) {
		t.Fatal("aborted session directory still exists")
	}
	n, err := st.CountItems(ctx, id)
	if err != nil {
		t.Fatal(err)
	}
	if n != 0 {
		t.Fatalf("aborted import left %d memory items", n)
	}
}
