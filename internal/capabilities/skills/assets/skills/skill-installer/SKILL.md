...
1. Determine the source — a git repository URL is best. Skills install into the user skill root (`~/.agents/skills`).
...
3. Use `tool_search` to locate the `skill_install` tool if it is not already available, then call it with `repo` and `path` (when needed).
