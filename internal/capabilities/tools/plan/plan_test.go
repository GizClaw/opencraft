...
func TestPlanPersistsAcrossStores(t *testing.T) {
	...
	// The plan lands in the shared SQLite state, keyed by agent.
	sess := mustSessions(t, root)
	var byAgent map[string]Plan
	if err := sess.ReadState("s-1", "plans", &byAgent); err != nil {
		t.Fatalf("read plans state: %v", err)
	}
	if _, ok := byAgent["assistant"]; !ok {
		t.Fatalf("plans state missing agent key: %+v", byAgent)
	}
...
func TestPlansFileKeyedByAgent(t *testing.T) {
	...
	// Both agent entries present in the shared SQLite state.
	sess := mustSessions(t, root)
	var byAgent map[string]Plan
	if err := sess.ReadState("s-1", "plans", &byAgent); err != nil {
		t.Fatal(err)
	...
	if len(byAgent) != 2 {
		t.Fatalf("plans agent entries = %d, want 2: %+v", len(byAgent), byAgent)
	}
