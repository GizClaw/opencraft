// Package httpclient carries the HTTP conveniences opencraft uses for
// web tools and provider clients: a tuned connection-pooling transport,
// a bounded retry RoundTripper for transient failures, and an
// option-driven client builder that hardens HTTP/1.1, HTTP/2, or HTTP/3.
//
// Ported from flowcraft sdkx/internal/httpkit (MIT licensed) so it can
// be imported outside the flowcraft module tree.
package httpclient
...
// WithProtocol selects the transport family.
...
// WithTimeout sets the whole-request timeout on the built client.
...
// WithResponseHeaderTimeout bounds the wait for HTTP/1.1 response headers.
...
// WithRetry sets a custom retry policy and enables retries.
...
// retries so an outer retry owner controls the full budget.
...
// WithoutRetry disables the retry wrapper.
...
// byte-slice bodies the clients send), so a streaming request retries
// connection and status failures but never re-opens mid-stream.
...
const retryCountHeader = "X-Opencraft-Retry-Count"
