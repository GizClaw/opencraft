...
func (a *App) Diagnostics() DiagnosticsReport {
	...
	rep := DiagnosticsReport{
		Version:   octelemetry.ServiceVersion,
	...
	if a.sessionStore() != nil {
		if metas, err := a.sessionStore().List(); err == nil {
	...
	usageStore := a.usage
	a.mu.Unlock()
	rep.ActiveRuns = a.activeRunCount()
...
func (a *App) ClearCaches() (CacheClearResult, error) {
	...
	if wd := a.snapshotWorkDir(); wd != "" {
		dirs = append(dirs, a.workspaceCacheDirFor(wd))
	}
	var bytes int64
...
func (a *App) execPolicyRules(wd string) ([]string, error) {
	...
	data, err := os.ReadFile(a.approvalsPathFor(wd))
	if err == nil {
