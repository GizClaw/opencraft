...
// <workDir>/.opencraft/undo/<contextID>/, split into live/ and
// undone/ so undo/redo move entries instead of rewriting them.
type Store struct {
	workDir string
	root    string
	mu      sync.Mutex
...
func New(workDir string) *Store {
	return &Store{
		workDir: workDir,
		root:    filepath.Join(workDir, ".opencraft", "undo"),
	}
}
