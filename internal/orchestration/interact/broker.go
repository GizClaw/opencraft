...
type Broker struct {
	rt         Runtime
	backend    Backend
	backendFor func(runID string) Backend
...
// used.
func NewWithBackendResolver(
	rt Runtime,
	fallback Backend,
	resolver func(runID string) Backend,
) *Broker {
	return &Broker{
		rt:         rt,
		backend:    fallback,
...
func (b *Broker) ask(ctx context.Context, spec Spec) {
	backend := b.backend
	if b.backendFor != nil {
		if alt := b.backendFor(spec.RunID); alt != nil {
			backend = alt
		}
	}
	reply, err := backend.Ask(ctx, spec)
	if err != nil {
