...
func TestBrokerRoutesPromptByRunBackend(t *testing.T) {
	rt := &fakeRuntime{}
	fallback := &fakeBackend{reply: Reply{Status: ReplyOK, Text: "fallback"}}
	auto := &fakeBackend{reply: Reply{Status: ReplyOK, Text: "auto"}}
	broker := NewWithBackendResolver(rt, fallback, func(runID string) Backend {
		if runID == "r-auto" {
			return auto
		}
		return nil
	...
	broker.BindTurn("t1", &fakeReplier{})

	rt.publish(t, session.PromptRequested{
		RunID: "r-auto", TurnID: "t1", PromptID: "p1",
		Prompt: agent.UserPrompt{Parts: []message.Part{
			message.TextPart{Text: "hi"},
		}},
	}, session.SubjectPromptRequested("r-auto"))
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("auto backend not called")
		}
		time.Sleep(time.Millisecond)
	}
	if len(fallback.specs) != 0 {
		t.Fatalf("fallback backend called for auto run: %+v", fallback.specs)
	}
}
