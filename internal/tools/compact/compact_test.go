...
func TestExecuteCondensesAndPersistsArtifact(t *testing.T) {
	...
	if out != "S1" {
		t.Fatalf("summary = %q, want S1", out)
	}
	if calls != 1 {
	...
	if out2 != "S1" || calls != 1 {
		t.Fatalf("reuse = %q calls=%d, want S1 calls=1", out2, calls)
	}
}
...
func TestExecuteMergesNewMessagesWithArtifact(t *testing.T) {
	...
	if out != "S2" {
		t.Fatalf("summary = %q, want S2", out)
	}
...
func TestExecuteSkipsSummaryMarkedMessages(t *testing.T) {
	...
	if out != "S1" || calls != 1 {
		t.Fatalf("reuse = %q calls=%d, want S1 calls=1", out, calls)
	}
	...
	if out != "S2" || calls != 2 {
		t.Fatalf("next = %q calls=%d, want S2 calls=2", out, calls)
	}
}
